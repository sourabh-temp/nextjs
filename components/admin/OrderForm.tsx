...
function OrderForm({ onClose, onSubmit, editingOrder }: OrderFormProps) {
  ...
  const handleOrderStatusChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const newOrderStatus = e.target.value as 'paid' | 'unpaid'; // Type assertion
    setOrder({ ...order, orderStatus: newOrderStatus });
  };

  const handleDeliveryStatusChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const newDeliveryStatus = e.target.value as 'delivered' | 'in progress'; // Type assertion
